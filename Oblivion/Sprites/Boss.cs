...
namespace Oblivion
{
    public class Boss : Sprite
    {
        ...
        public void Update(GameTime gameTime, Dictionary<Vector2, Rectangle> collisionBlocks, Camera2D camera)
        {
            ...
            Console.WriteLine("Enemy Bounds " + _leftBound + " "+ _rightBound);
            ...
            int newAnimationRow = 3;
                                ...
                                // Apply gravity to vertical velocity BEFORE any movement
            ApplyGravity(deltaTime);

            // Horizontal movement and collision
            Position.X += _enemyVelocity.X * deltaTime;
            UpdateHitbox();
            HandleCollision(collisionBlocks); // resolve horizontal collisions

            // Vertical movement and collision
            Position.Y += _enemyVelocity.Y * deltaTime;
            UpdateHitbox();
            HandleCollision(collisionBlocks); // resolve vertical collisions
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            Vector2 origin = new Vector2(
                _animation.GetSourceRect().Width / 2f,
                _animation.GetSourceRect().Height / 2f
            );

            spriteBatch.Draw(
                _texture,
                Position + origin  * HitboxScale,
        ...
        private void ChasePlayer(float deltaTime)
        {
            ...
            float bossCenterX = _hitbox.Center.X;
            float playerCenterX = _player.Hitbox.Center.X;
            float flipThreshold = 100f;
            if (Math.Abs(playerCenterX - bossCenterX) > flipThreshold)
            {
