...
namespace Oblivion
{
    public class Player : Sprite
    {
        ...
        public void Update(GameTime gameTime, Dictionary<Vector2, Rectangle> collisionBlock)
        {
            ...
            foreach (var tile in collisionBlock.Values)
            {
                if (hitbox.Intersects(tile))
                {
                    //Top
                    if (velocity.Y >= 0 && hitbox.Bottom - velocity.Y <= tile.Top)
                    {
                        Position.Y = tile.Top - hitbox.Height;
                        velocity.Y = 0;
                        UpdateHitbox();
                    }
                    else if (velocity.Y <= 0 && hitbox.Top - velocity.Y <= tile.Bottom)
                    {
            ...
            foreach (var tile in collisionBlock.Values)
            {
                if (hitbox.Intersects(tile))
                {
                    if (velocity.X >= 0 && hitbox.Right - velocity.X <= tile.Left)
                        Position.X = tile.Left - hitbox.Width;
                    else if (velocity.X <= 0 && hitbox.Left - velocity.X <= tile.Right)
        ...
        {
        hitbox = new Rectangle((int)Position.X, (int)Position.Y, (int)(_animation.FrameWidthAccess*2f), (int)(_animation.FrameHeightAccess*2f));
        }
    }
}
